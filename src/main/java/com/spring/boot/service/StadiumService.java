...
public class StadiumService implements IStadiumService {
    ...
    public HashMap<String,Object> verifyStrings(StadiumJson stadium, IStadiumRepository iStadiumDAO, HashMap<String, Object> map){
    ...
    public HashMap<String,Object> verifyNumericPut(StadiumJson s, HashMap<String,Object> map){
        ...
        try{
            ...
            if ( !s.getId().isEmpty() ) Long.parseLong(s.getId());
            else {
                map = messageError(" Id");
                return map;
            }
    ...
    public Stadium addMatch(Stadium stadium, Optional<Match> match){
        Match x = new Match();
        // En esta Demo, no esta ligado a to do el proyecto.
//        if ( match.isPresent()){
//            x = match.get();
//        }
